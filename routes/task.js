...
const ftp = new jsftp({
  ...
  user: "portal@onlinedatacapture.com", // defaults to "anonymous"
  ...
  mytaskwp: (req, res) => {
    ...
    db.query(usernameQuery, (err1, result1) => {
      ...
      if (result1.length > 0) {
        ...
        return res.status(200).json({ message: 'No record found', status: 200, final_array: [] });
      }
    });
  },
      ...
      if (result.length > 0) {
        ...
        db.query(usernameQuery1, (err1, result1) => {
          ...
          if (result1.length > 0) {
            ...
            let usernameQuery2 = "UPDATE `wp_posts` SET  `post_date_gmt`='" + datecurrent + "', `post_content`=concat('" + postcontent + "',ifnull(post_content,'')), `comment_count`=comment_count+1 WHERE ID='" + wppostID + "'";
            db.query(usernameQuery2, (err2, result2) => {
              ...
              let usernameQuery3 = "INSERT INTO `wp_comments` ( `comment_post_ID`, `comment_author`, `comment_author_email`, `comment_author_url`, `comment_author_IP`, `comment_date`, `comment_date_gmt`, `comment_content`, `comment_karma`, `comment_approved`, `comment_agent`, `comment_type`, `comment_parent`, `user_id`) VALUES ('" + wppostID + "', '" + Usrauther + "', '" + Usremail + "', '" + Usrurl + "', '" + strIPClient + "', '" + datecurrent + "', '" + datecurrent + "','" + postcontent + "', '0', '1', '', '', '0', '" + userid + "')";
...
*/
    ...
    let usernameQuery1 = "SELECT * FROM `wp_terms` WHERE `term_group` = '1'";
    db.query(usernameQuery1, (err, result) => {
      if (err) {
        return res.status(500).json({ message: 'errr5', status: 500, msg: err, wpstatus: 0 });
      }
  ...
  taskactivewp: (req, res) => {
    ...
    let TotalPost = 0;
    let NewPost = 0;
    let usernameQuery1 = " SELECT count(*) as countsno FROM `wp_posts` WHERE `post_author` = '" + userid + "' and post_type='fast_ticket'";
    db.query(usernameQuery1, (err, result) => {
      if (err) { }
      TotalPost = result[0].countsno;
      let usernameQuery = " SELECT count(*) as countsno FROM `wp_posts` a join wp_comments b on a.ID=b.comment_post_ID WHERE a.`post_author` = '" + userid + "' and b.user_id !='" + userid + "' and a.post_content !='' and b.read_comment='0' group by ID";
      db.query(usernameQuery, (err1, result1) => {
        if (err1) { }
        NewPost = result1.length;
        return res.status(200).json({ status: 200, newpost: NewPost, TotalPost: TotalPost, wpstatus: 1 });
      });
    });
    //setTimeout(() =>{       
    //  return res.status(200).json({ status :200, newpost:NewPost,TotalPost:TotalPost,wpstatus:1,NewNotify:NewNotify,NewReviews:NewReviews });
    //}, 2000);     
  },


  taskactiveDashwp: (req, res) => {
    ...
    db.query(Query2, (err2, result2) => {
      ...
      db.query(query3, (err3, result3) => {
        ...
        return res.status(200).json({ status: 200, wpstatus: 1, NewNotify: NewNotify, NewReviews: NewReviews });
      });
    });
  },
  ...
  taskimgwp: (req, res) => {
    ...
    var addnewfile = timestamp + ext;
  
    //-----------------Zaheer Code----------------------//       
   
    var fileBuffer = uploadedFile;//Buffer.from(addnewfile, 'base64')
    var remote = folderName + currentTime.getFullYear() + "/0" + currentTime.getMonth() + "/" + addnewfile;
    //var fileBuffer = Buffer.from('fileName', 'base64')
    ...
    ftp.put(fileBuffer, remote, function (err) {
      if (err) {
        ...
        console.log(addnewfile + " - uploaded successfuly");
        //callback();
      }
    });
    //-----------------Zaheer Code end----------------------//  
  ...
  taskcreatewp: (req, res) => {
    ...
    db.query(usernameQuery, (err, result) => {
      ...
      if (result.length > 0) {
        ...
        let Query1 = " SELECT * FROM `wp_posts` WHERE `post_title` = '" + posttitle + "' and post_author = '" + userid + "' ";
          ...
          if (result11.length > 0) {
            ...
            */
            ...
            db.query(usernameQuery1, (err1, result1) => {
              ...
              db.query(usernameQuery102, (err4, result4) => {
              ...
              db.query(usernameQuery2, (err2, result2) => {
                ...
                // if post conntent will not be blank after that it will be call to wp_comments table .
                if (postcontent != "<p><p>") {
                  console.log("if cons..")
                  var strIP = localStorage.getItem('ipInfo');
                  var strIPClient = JSON.parse(strIP).clientIp;
                  let usernameQuery3 = "INSERT INTO `wp_comments` ( `comment_post_ID`, `comment_author`, `comment_author_email`, `comment_author_url`, `comment_author_IP`, `comment_date`, `comment_date_gmt`, `comment_content`, `comment_karma`, `comment_approved`, `comment_agent`, `comment_type`, `comment_parent`, `user_id`,comment_description) VALUES ('" +
                    wppostID + "', '" + Usrauther + "', '" + Usremail + "', '" + Usrurl + "', '" + strIPClient + "', '" + datecurrent + "', '" + datecurrent + "','" + postcontent + "', '0', '1', '', '', '0', '" + userid + "','" + postcontent + "')";
                  db.query(usernameQuery3, (err3, result3) => {
                    if (err3) {
                  ...
                  return res.status(200).json({ message: "Your task has been saved successfully.", status: 200, wpstatus: 1, commentID: "" });
                }
  ...
  */
    ...
    db.query(qry2, (err2, result2) => {
      ...
      db.query(qry4, (err4, result4) => {
        ...
        post_date = dateFormat(result4[0].post_date, "yyyy-mm-dd h:MM:ss");

        post_date_gmt = dateFormat(result4[0].post_date_gmt, "yyyy-mm-dd h:MM:ss");
  ...
  tasknotifyUp: (req, res) => {

    let userid = req.body.userid;
    let postid = req.body.postid;
    let notifystatus = req.body.notifystatus;

    let usernameQuery = "SELECT * FROM `wp_posts` WHERE `ID` = '" + postid + "'";
    db.query(usernameQuery, (err1, result1) => {
      if (err1) {
        return res.status(500).json({ message: 'errr5', status: 500, msg: err1, wpstatus: 0 });
      }
      if (result1.length > 0) {
        // If open_comment    will be 1 then their will not be received notification from user end . If open_comment    will be 0 then their will be  received notification from user end
        let qry2 = "UPDATE `wp_posts` set open_comment= '" + notifystatus + "' where ID= '" + postid + "'";
        db.query(qry2, (er2, result2) => {
          if (er2) {
            return res.status(500).json({ message: 'errr5', status: 500, msg: er2, wpstatus: 0 });
          }
          return res.status(200).json({ status: 200, message: "Data updated successfully.", wpstatus: 1 });
        });
      }
      else {
        return res.status(200).json({ message: 'Task is not available.', status: 200, wpstatus: -1 });
      }
    });
  },
